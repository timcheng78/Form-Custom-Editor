...
export class RadioComponent extends BaseComponent {
    static dragName        = 'radio'
    static dragDescription = '單選方塊'
    constructor (name, bean, title) {
        /** 建構式創建 */
        super('form-group pFormItem', "true", name, bean, title, "true", 'Y', "false", "true", "radio", "true")
        /** 各類型客製化預設設置 */
    ...
    generateBeforeChanged() {
        ...
        for (let i = 0; i < count; ++i) {
            ...
            cloneDescription.div.children[0].input.value    = valueArray[i] || '選項'
            cloneDescription.div.children[0].input.id       += i
            cloneDescription.div.children[1].label.for      += i
            cloneDescription.div.children[1].label.text     = descArray[i] || '選項'
        ...
        switch (attributeName) {
            case ATTRIBUTE_NAMES.NAME:
                this.componentElement.forEach(element => {
                    const radio = element.querySelector('.form-check-input')
                    radio.name = actualValue
                })
                break
            case ATTRIBUTE_NAMES.OPTION_EDIT:
            ...
            case ATTRIBUTE_NAMES.UI_CLASS:
                this.componentElement.forEach(element => {
                    const radio = element.querySelector('.form-check-input')
                    if (actualValue === '' || actualValue === '0') radio.classList.value = 'form-check-input radio-default'
                    else radio.classList.add(actualValue)
                })
                break
        }
    }
    ...
    makingOption() {
        this.componentElement.forEach((element, index) => {
            ...
            radio.value = uiValue[index]
            radio.checked = checked.split(',')[index] === 'true'
        })
        try {
     ...
     */
    checkingByItself() {
        // radio setting horizontalFormItem (設定選取框的展開元件屬性)
        ...
        /** 取得選項底下的全部元件 */
        const radioBeans     = this.fullComponent.querySelectorAll('.pFormItem[data-name], .pFormItemGroup[data-name]')
        radioBeans.forEach(bean => {
            // radio 結構為兩層到父層
